...
def round_int(x):
    return tf.cast(tf.round(x), tf.int64)
...
def read_example(rec_queue, pr, input_types):
    ...
    if "sfs" in input_types:
        feats["sfs"] = tf.FixedLenFeature(
            [pr.full_specgram_samples * pr.num_mel], dtype=tf.float32
        )
    ...
    if not pr.im_split:
        ...
        if pr.num_splits == 2 and (not hasattr(pr, "new_split") or not pr.new_split):
            total_frames = (
                pr.total_frames
                if not pr.variable_frame_count
                ...
                map(
                    ...
                    [
                        tf.image.decode_jpeg(
                            example["im_0"], channels=3, name="decode_im1"
                        ),
                        tf.image.decode_jpeg(
                            example["im_1"], channels=3, name="decode_im2"
                        ),
                    ],
                ),
                0,
    ...
    if pr.variable_frame_count:
        ims = tf.reshape(
            ims, tf.concat([[total_frames], [full], [full], [3]], 0), name="reshape_ims"
        )
    else:
    ...
    if "flow" in input_types:
        flows = tf.image.decode_jpeg(example["flow"], channels=3, name="decode_flow")[
            :, :, :2
        ]
        flows.set_shape((pr.full_flow_dim * total_frames, pr.full_flow_dim, 2))
        flows = tf.reshape(flows, (total_frames, pr.full_flow_dim, pr.full_flow_dim, 2))
    else:
    ...
    if pr.variable_frame_count:
        samples = tf.reshape(
            samples, (tf.shape(samples)[0] / 2, 2), name="reshape_samples"
        )
    else:
    ...
    if pr.do_shift:
        ...
        shift_frame = start_frame_gt = tf.random_uniform(
            [1], 0, total_frames - num_slice_frames, dtype=tf.int32
        )[0]
    ...
    if pr.augment_ims:
        ...
        r = tf.random_uniform([2], 0, shape(ims, 1) - pr.crop_im_dim, dtype=tf.int32)
        x, y = r[0], r[1]
        # x = tf.Print(x, [x, y])
    else:
        if hasattr(pr, "resize_dims"):
    ...
    if pr.augment_ims:
        ims = tf.cond(
            tf.cast(tf.random_uniform([1], 0, 2, dtype=tf.int64)[0], tf.bool),
    ...
    def slice_samples(frame):
        ...
        if pr.augment_audio:
            ...
            delta = tf.random_uniform(
                [1],
                long(-dist * pr.samples_per_frame),
                long(dist * pr.samples_per_frame),
                dtype=tf.int64,
            )[0]
            start = start + delta
            ...
            start = tf.minimum(
                start,
                tf.cast(
                    cast_int(cast_float(total_frames) * pr.samples_per_frame)
                    - num_samples
                    - 1,
                    tf.int64,
                ),
...
def make_db_reader(
    ...
    if hasattr(pr, "alt_tf_path") and pr.alt_tf_path is not None:
        ...
        rec_files = [
            (x.replace(pr.alt_tf_path[0], pr.alt_tf_path[1]) if i % 2 == 0 else x)
            for i, x in enumerate(rec_files)
        ]
    ...
    queues = [
        tf.train.string_input_producer(
            ...
            num_epochs=(1 if one_pass else None),
        )
    ...
    if not one_pass and (pr.dset_seed is None):
        ...
        ims, flows, samples, sfs, labels, ytids = tf.train.batch(
            example_list[0], batch_size
        )
