...
class NetClf:
            ...
            with tf.device(self.gpu):
                ...
                if self.restore_only_shift:
                    ...
                    var_list = [
                        x
                        for x in var_list
                        if x.name.startswith("im/")
                        or x.name.startswith("sf/")
    ...
    def predict_unmixed(self, ims, samples0, samples1):
        ...
        return dict(
            samples_pred_fg=samples_pred_fg[0],
...
def crop_from_cam(ims, cam, pr):
    cam = np.array([ig.blur(x, 2.0) for x in cam])
    cam = np.abs(cam)
    cam = cam.mean(0)
...
def run(vid_file, start_time, dur, pr, gpu, buf=0.05, mask=None, arg=None, net=None):
    ...
    with ut.TmpDir() as vid_path:
        height_s = (
            "-vf \"scale=-2:'min(%d,ih)'\"" % arg.max_full_height
            if arg.max_full_height > 0
            else ""
        )
        ut.sys_check(
            ut.frm(
...
if __name__ == "__main__":
    ...
    arg.add_argument(
        ...
        type=float,
        default=None,
        help="Multiply the default duration of the audio (i.e. %f) by this amount. Should be a power of 2."
        % sep_params.VidDur,
    )
    arg.add_argument(
        "--mask",
    ...
    arg.start = ut.make_mod(arg.start, (1.0 / pr.fps))
    ...
    for t in ut.time_est(ts):
        t = ut.make_mod(t, (1.0 / pr.fps))
        frame_start = int(t * pr.fps - arg.start * pr.fps)
        ret = run(
            arg.vid_file, t, arg.clip_dur, pr, gpus[0], mask=arg.mask, arg=arg, net=net
        )
        if ret is None:
            continue
        ims = ret["ims"]
        ...
        filled[inds] = True
    full_samples_fg = np.clip(full_samples_fg, -1.0, 1.0)
    ...
    table = [
        ["start =", arg.start],
        "fg:",
        imtable.Video(full_ims, pr.fps, Sound(full_samples_fg, pr.samp_sr)),
        "bg:",
        imtable.Video(full_ims, pr.fps, Sound(full_samples_bg, pr.samp_sr)),
        "src:",
        imtable.Video(full_ims, pr.fps, Sound(full_samples_src, pr.samp_sr)),
    ]
    ...
    if arg.out is not None:
        ...
        ut.make_video(
            full_ims, pr.fps, pj(arg.out, "fg%s.mp4" % name), snd(full_samples_fg)
        )
        ut.make_video(
            full_ims, pr.fps, pj(arg.out, "bg%s.mp4" % name), snd(full_samples_bg)
        )
        ut.make_video(
            full_ims, pr.fps, pj(arg.out, "src%s.mp4" % name), snd(full_samples_src)
        )
    else:
