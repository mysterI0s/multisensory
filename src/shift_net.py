...
# 1. Aliases TF1 layers to fix scope duplication.
# 2. Disables 'renorm' to fix missing checkpoint keys.
# ---------------------------------------------------------

# Force Legacy Mode
os.environ["TF_USE_LEGACY_KERAS"] = "1"

if "tf_keras.legacy_tf_layers" not in sys.modules:
    # A. Create the parent package
    legacy_pkg = types.ModuleType("tf_keras.legacy_tf_layers")
    legacy_pkg.__path__ = []
    sys.modules["tf_keras.legacy_tf_layers"] = legacy_pkg

    # B. Define a Safe BatchNormalization class
    # This class forces 'renorm=False' to prevent looking for missing variables.
    class SafeBatchNormalization(tf.compat.v1.layers.BatchNormalization):
        def __init__(self, *args, **kwargs):
            # Force disable renorm because the checkpoint lacks renorm variables
            if kwargs.get("renorm", False):
                print("(!) Forced renorm=False for compatibility with checkpoint.")
                kwargs["renorm"] = False
            super().__init__(*args, **kwargs)

    # C. Helper to alias modules
    def alias_module(mod_name, class_map):
        mod = types.ModuleType("tf_keras.legacy_tf_layers." + mod_name)
        sys.modules["tf_keras.legacy_tf_layers." + mod_name] = mod
        setattr(legacy_pkg, mod_name, mod)

        for keras_name, target in class_map.items():
            if isinstance(target, str):
                # Map string name to tf.compat.v1.layers class
                if hasattr(tf.compat.v1.layers, target):
                    setattr(mod, keras_name, getattr(tf.compat.v1.layers, target))
            else:
                # Map directly to a class (like our SafeBatchNormalization)
                setattr(mod, keras_name, target)

    # D. Map 'normalization' -> Our SafeBN
    alias_module("normalization", {"BatchNormalization": SafeBatchNormalization})

    # E. Map 'core' -> Standard TF1 layers
    alias_module("core", {"Dense": "Dense", "Dropout": "Dropout", "Flatten": "Flatten"})

    # F. Map 'convolutional' -> Standard TF1 layers
    alias_module(
        "convolutional",
        {
            "Conv2D": "Conv2D",
...
try:
    from . import tfutil as mu
    from . import aolib
    from .aolib import util as ut
    from . import shift_dset
except ImportError:
    import tfutil as mu
    ...
    with tf.device("/cpu:0"):
        ...
        ims, samples = mu.on_cpu(
            lambda: shift_dset.make_db_reader(
                pr.train_list, pr, batch, ["im", "samples"], num_db_files=pr.num_dbs
            )
        )
...
def arg_scope_2d(
    ...
    with slim.arg_scope([slim.batch_norm], **bn_prs):
        with slim.arg_scope(
            ...
            normalizer_fn=normalizer_fn,
            reuse=reuse,
            normalizer_params=normalizer_params_2d,
        ) as sc:
            return sc
...
class Model:
        ...
        with tf.device(self.default_gpu):
            ...
            for i, gpu in enumerate(self.gpus):
                with tf.device(gpu):
                    ...
                    with tf.device("/cpu:0"):
                        ...
                        samples0 = tf.where(
                            tf.equal(labels, 1), samples_ex[:, 1], samples_ex[:, 0]
                        )
                        samples1 = tf.where(
                            tf.equal(labels, 0), samples_ex[:, 1], samples_ex[:, 0]
                        )
                        labels1 = 1 - labels

                    net0 = make_net(
                        ims, samples0, pr, reuse=reuse, train=self.is_training
                    )
                    net1 = make_net(
                        None,
                        samples1,
            ...
            if not self.is_training:
                ...
                if pr_test.do_shift:
                    ...
                    self.test_samples = tf.where(
                        tf.equal(self.test_labels, 1),
                        self.test_samples[:, 1],
                        self.test_samples[:, 0],
                    )
                else:
            ...
            gs = [
                mu.print_every(gs[0], 100, ["grad norm:", tf.global_norm(gs)])
            ] + list(gs[1:])
            gvs = zip(gs, vs)
            ...
            if pr.multipass:
                ops = [
                    ...
                    for i in range(pr.multipass_count)
                ]
    ...
    def restore(
        ...
        path=None,
        restore_opt=True,
        restore_resnet18_blocks=True,
        restore_dilation_blocks=True,
    ):
        if path is None:
        ...
        if not restore_opt:
            var_list = [
                x for x in var_list if not any(name in x.name for name in opt_names)
            ]
    ...
    def train(self):
        ...
        while True:
            ...
            if step % pr.summary_iters == 0:
                ret = self.sess.run(
                    [self.train_op(), self.merged_summary] + self.loss.get_losses()
                )
                self.sum_writer.add_summary(ret[1], step)
                loss_vals = ret[2:]
            else:
                loss_vals = self.sess.run([self.train_op()] + self.loss.get_losses())[
                    1:
                ]
...
def train(pr, gpus, restore=False, restore_opt=True):
    ...
    with tf.Graph().as_default():
        ...
        if restore:
            ...
            model.restore(
                pr.init_path, restore_resnet18_blocks=False, restore_opt=False
            )
...
def block2(net, nf, dim, scope, stride=1, reuse=None):
    ...
    if stride != 1 and shape(net, -1) == nf:
        ...
        short = conv2d(
            net, nf, [1, 1], scope="%s_short" % scope, stride=stride, activation_fn=None
        )

    net = conv2d(net, nf, dim, scope="%s_1" % scope, stride=stride)
    net = conv2d(
        net, nf, dim, scope="%s_2" % scope, activation_fn=None, normalizer_fn=None
    )
    net = short + net
    net = slim.batch_norm(
        net, scope="%s_bn" % scope, activation_fn=tf.nn.relu, reuse=reuse
    )
    return net


def block3(
    net, nf, dim, scope, stride=1, rate=1, reuse=None, bottleneck=False, use_bn=True
):
    short = net
    if stride != 1 and shape(net, -1) == nf:
...
def merge(sf_net, im_net, pr, reuse=None, train=True):
    ...
    if train:
        ...
        sf_net = tf.nn.fractional_max_pool(
            sf_net,
            [1, float(shape(sf_net, 1) - 1) / shape(im_net, 1), 1, 1],
            deterministic=(not train),
            pseudo_random=(not train),
            seed=(0 if train else 1),
            seed2=(0 if train else 2),
        )[0]
...
def make_net(ims, sfs, pr, im_net=None, reuse=True, train=True):
    ...
    with slim.arg_scope(arg_scope_3d(pr=pr, reuse=reuse, train=train)):
        ...
        net = block3(
            net, 512, [3, 3, 3], "im/conv5_1", stride=[time_stride, s, s], reuse=reuse
        )
        ...
        logits = conv3d(
            net, 1, [1, 1, 1], scope=logits_name, activation_fn=None, normalizer_fn=None
        )[:, 0, 0, 0, :]
        cam = conv3d(
            last_conv,
            1,
...
class NetClf:
    ...
    def init(self, reset=True):
        if self.sess is None:
            ...
            with tf.device(self.gpu):
                ...
                self.net = make_net(
                    self.ims_ph, self.samples_ph, pr, reuse=False, train=False
                )
                ims_resize = self.ims_resize_ph
                ims_resize = ed(
                    tf.image.resize_images(
                        ims_resize[0], (pr.crop_im_dim, pr.crop_im_dim)
                    ),
                    0,
                )
                ims_resize.set_shape(shape(self.ims_ph))
                self.net_resize = make_net(
                    ims_resize, self.samples_ph, pr, reuse=True, train=False
                )
    ...
    def predict_cam(self, ims, samples):
        self.init()
        cam = self.sess.run(
            [self.net.cam], {self.ims_ph: ims, self.samples_ph: samples}
        )
        return cam
    ...
    def predict_cam_resize(self, ims, samples):
        self.init()
        cam = self.sess.run(
            [self.net_resize.cam], {self.ims_resize_ph: ims, self.samples_ph: samples}
        )
        return cam
