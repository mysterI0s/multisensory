...
def read_example(rec_queue, pr, input_types):
    ...
    im_parts = map(
        f,
        [
            tf.image.decode_jpeg(example["im_0"], channels=3, name="decode_im1"),
    ...
    if pr.augment_ims:
        ...
        r = tf.random_uniform([2], 0, pr.full_im_dim - pr.crop_im_dim, dtype=tf.int32)
        x, y = r[0], r[1]
        # x = tf.Print(x, [x, y])
    else:
    ...
    slice_parts = []
    for j in range(len(im_parts)):
        num_frames_in_part = total_frames / len(im_parts)

        part_start = j * num_frames_in_part
        frame_offset = tf.maximum(
            0, tf.minimum(start_frame_gt - part_start, num_frames_in_part)
        )
        end_offset = tf.maximum(
            0,
            tf.minimum(
                start_frame_gt + num_slice_frames - part_start, num_frames_in_part
            ),
        )
