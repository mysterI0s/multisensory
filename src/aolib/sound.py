...
class converted_wav:
    ...
    def __enter__(self):
        ...
        if 0 != os.system(
            'ffmpeg -loglevel warning -i "%s" -acodec pcm_s32le "%s"'
            % (self.in_fname, self.out_fname)
        ):
...
def load_sound(wav_fname):
    ...
    times = (1.0 / rate) * np.arange(len(samples))
...
def load_sound_uint8(wav_fname):
    ...
    samples = np.array(samples, "float32") / 128.0 - 0.5
    times = (1.0 / rate) * np.arange(len(samples))
    return Sound(times, rate, samples)
...
def vis_specgram_at_times(
    snd, times, window_sec, par=0, im_shape=(500, 888), compress_ims=False
):
    snd = snd.normalized().to_mono()
    ...
    def f(t, spec=spec, window_sec=window_sec, im_shape=im_shape):
        ...
        time_lo = t - window_sec / 2.0
        time_hi = t + window_sec / 2.0
        vsc = vis_specgram(
            freqs, sft, sft_times, time_lo=time_lo, time_hi=time_hi, plot_fn=plot_fn
        )
...
def make_specgram(
    sound, rate, shift_fft=True, sample_times=None, nfft=None, noverlap=2000, par=0
):
    ...
    freqs = np.fft.fftfreq(sft.shape[1], 1.0 / rate)
...
def test_spectrogram():
    ...
    ax2.imshow(
        lsp.T,
        cmap=pl.cm.jet,
        extent=(0.0, t[-1], np.min(freqs), np.max(freqs)),
        aspect="auto",
    )
...
def pink_noise(n, scale=1.0, alpha=1.0):
    # not exactly pink
    if n <= 1:
        return np.randn(n)
    spec = np.random.randn(n)
    # power \prop 1/f^alpha ==> sqrt(power) \prop 1/sqrt(f^alpha)
    spec = 1.0 / (scale * np.sqrt(np.arange(1, n + 1) ** alpha)) * spec
...
class Sound:
    ...
    def __getslice__(self, *args):
        return Sound(
            self.times.__getslice__(*args), self.rate, self.samples.__getslice__(*args)
        )
    ...
    def normalized(self, check=True):
        if self.samples.dtype == np.double:
            assert (not check) or np.max(np.abs(self.samples)) <= 4.0
            x = copy.deepcopy(self)
            x.samples = np.clip(x.samples, -1.0, 1.0)
            return x
        else:
    ...
    def unnormalized(self, dtype_name="int32"):
        ...
        samples = np.clip(s.samples, -1.0, 1.0)
    ...
    def left_pad_silence(self, n):
        if n == 0:
            ...
            if np.ndim(self.samples) == 1:
                ...
                samples = np.vstack(
                    [
                        np.zeros((n, self.samples.shape[1]), self.samples.dtype),
                        self.samples,
                    ]
                )
    ...
    def resampled(self, new_rate, clip=True):
        ...
        if new_rate == self.rate:
            ...
            return Sound(
                None,
                new_rate,
                subband.resample(self.samples, float(new_rate) / self.rate, clip=clip),
            )
...
def concat_sounds(snds, par=1):
    ...
    if ut.ndistinct(ut.mapattr(snds).rate) == 1:
        return Sound(
            None, snds[0].rate, np.concatenate(ut.mapattr(snds).samples, axis=0)
        )
    else:
...
def convert_sound_compat(
    in_fname, out_fname, duration=None, rate=None, codec="pcm_s32le", ffmpeg_flags=""
):
    ...
    with ut.temp_file(".mp3") as mp3_tmp, ut.temp_file(".wav") as wav_tmp:
        ok = 0 == ut.sys_print(
            'ffmpeg -i "%s" %s -y %s "%s"'
            % (in_fname, duration_flag, ffmpeg_flags, mp3_tmp)
        )
        ...
        ok = ok and (
            0
            == os.system(
                'ffmpeg -i "%s" -y -ac 1 -acodec %s %s "%s"'
                % (mp3_tmp, codec, ar_str, wav_tmp)
            )
        )
        if ok:
...
def audio_sampling_rate(fname):
    return int(
        ut.sys_with_stdout("ffprobe -show_streams %s | grep sample_rate" % fname).split(
            "sample_rate="
        )[1]
    )
