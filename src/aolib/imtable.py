...
def cycle_html(im_fnames_table, group_name, start):
    ...
    default = (
        im_fnames_table[start[0]][start[1]]
        if len(im_fnames_table) and len(im_fnames_table[0])
        else ""
    )
    ...
    return ut.frm(
        '<img src = "%(default)s" id = "%(id)s" onmouseover = \'curr_cycle_group = "%(group_name)s";\''
...
def paginate_table(table_rows, rows_per_page, title, output_dir):
    ...
    page_names = ["index.html"] + [
        "page_%d.html" % i for i in range(2, 1 + len(split_table))
    ]
    ...
    for i in range(len(split_table)):
        ...
        if len(split_table) == 1:
            ...
            footer = ""
            footer += (
                "Back " if i == 0 else "<a href = '%s'>Back</a> " % page_names[i - 1]
            )
            footer += (
                "Next "
                if i == -1 + len(page_names)
                else "<a href = '%s'>Next</a> " % page_names[i + 1]
            )
...
def html_from_cell(x, output_dir):
    if type(x) == type(""):
        ...
        if x[0] in ("img", "img_mv", "img-png"):
            ...
            return cycle_html(
                [[path_from_im(im, output_dir) for im in row] for row in im_table],
                group,
                start,
            )
            ...
            return "<table border = 1>%s</table>" % (
                "<tr>".join(html_from_rows(x[1], output_dir))
            )
...
def make_temp_animated_gif_cmdline(
    seq, duration=0.5, dir=None, tmp_ext=".ppm", seq_fname=None
):
    ...
    ut.prn_sys(
        "convert -layers OptimizePlus -delay %f -loop 0 %s_*%s %s"
    ...
    for row in table:
        ...
        if type(row) == type((1,)) and row[0] == "break":
            ...
            if type(row) != type([]):
                row = [row]
            html_rows.append(
                "<td>" + "<td>".join(html_from_cell(x, output_dir) for x in row)
            )
...
def test():
    ...
    show(
        [
            (
                "table",
                [["hello", "world"], "asdf", "fdsa", ["a", "b", "c", "d", "e", "f"]],
            ),
            ("table", [["foo", "bar"], ["x"]]),
        ]
    )
...
def make_video(out_fname, ims, fps, tmp_ext=".ppm", sound=None):
    ...
    with ut.temp_file(".wav") as tmp_wav:
        ...
        cmd = (
            "ffmpeg -loglevel error -f image2 -r %f -i %s/ao-video-frame%%05d%s %s "
            "-pix_fmt yuv420p -ar 22050 -vcodec h264 -strict -2 -y %s"
...
def test_video_cycle():
    ims = [
        "/afs/csail.mit.edu/u/a/aho/bear.jpg",
...
def sync_with_fps(ims, timestamps, target_fps, max_lag_sec=0.06):
    ...
    while i < len(ims):
        vid.append(ims[i])
        video_time = lambda: (len(vid) - 1) * (1.0 / target_fps)
...
class Cycle:
    ...
    def make_html(self, output_dir):
        ...
        return cycle_html(
            [[path_from_im(im, output_dir) for im in row] for row in ims],
            self.group,
            self.start,
        )
