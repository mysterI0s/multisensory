...
def dropout_prob(pr):
    return 0.5 if pr.dropout else 1.0


def weight_decay(pr):
    return slim.l2_regularizer(pr.weight_decay)
...
def layer_params(pr, train=True, reuse=None):
    ...
    if pr.init_method == "xavier":
        init_conv = tf.layers.xavier_initializer_conv2d()
        init_fc = tf.layers.xavier_initializer()
    ...
    dropout_scope = slim.arg_scope(
        [slim.dropout], keep_prob=dropout_prob(pr), is_training=train
    )
...
def mdn_loss_indep(
    mix, mu, log_sigma_sq, x, b, nc, k, nx, smooth=None, mean_loss=True, mean_dim=False
):
    cns = tf.constant
    ...
    combine_dims = (
        (lambda x: tf.reduce_mean(x, 2))
        if mean_dim
        else (lambda x: tf.reduce_sum(x, 2))
    )
    if mean_loss:
...
def make_mdn_indep(
    fc, conv_target, pr, scope, use_gpu=True, mean_loss=True, mean_dim=False
):
    scope = scope + "_"
    ...
    if pr.use_stdev:
        log_sigma_sq = slim.fully_connected(
            fc_regress,
            nc * dim,
            activation_fn=None,
            scope=scope + "log_sigma_sq",
            biases_initializer=tni(mean=np.log(1.0**2), stddev=0.1),
            weights_regularizer=weight_decay(pr),
        )
    ...
    if pr.mix_only:
        mu = tf.get_variable(
            scope + "mu", [batch_size, nc * dim], initializer=tni(mean=0.0, stddev=0.1)
        )
    else:
        mu = slim.fully_connected(
            ...
            scope=scope + "mu",
            activation_fn=None,
            biases_initializer=tni(mean=0.0, stddev=0.1),
            weights_regularizer=weight_decay(pr),
        )
...
def maybe_add_n(xs):
    return tf.add_n(xs) if len(xs) else tf.constant(0.0, dtype=tf.float32)
    ...
    def total_loss(self):
        if self.total_loss_ is None:
            ...
            for x in [x for x in self.losses if not ut.haseq(x, "ignore", True)]:
                print(x)
            self.total_loss_ = maybe_add_n(
                [x for x in self.losses if not ut.haseq(x, "ignore", True)]
            )
        # with tf.device('/cpu:0'):
...
def get_step():
    ...
    step = ut.find_first(
        lambda x: x.name.startswith("global_step:"), tf.global_variables()
    )
    return step
...
def print_every(ret, num_iters, vals, **kwargs):
    with tf.device("/cpu:0"):
        ...
        if step is not None:
            return tf.cond(
                tf.equal(tf.mod(step, num_iters), 0),
                lambda: tf.Print(ret, vals, **kwargs),
                lambda: ret,
            )
        else:
            return ret
...
def find_var(name):
    return ut.find_first(
        lambda x: x.name.startswith("%s:" % name), tf.global_variables()
    )
...
def label_loss(logits, labels, smooth=False, smooth_prob=0.05):
    if smooth:
        ...
        oh = smooth_prob * (1.0 / nc) + (1 - smooth_prob) * oh
        loss = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=oh)
    else:
...
def conv2d_same(inputs, num_outputs, kernel_size, stride, rate=1, scope=None, **kwargs):
    ...
    is equivalent to
       net = tf.layers.conv2d(inputs, num_outputs, 3, stride=1,
       padding='SAME')
       net = subsample(net, factor=stride)
    whereas
       net = tf.layers.conv2d(inputs, num_outputs, 3, stride=stride,
       padding='SAME')
